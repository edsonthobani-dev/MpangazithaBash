...
namespace MpangazithaBash.Repository
{
    public class GenericRepository<Tbl_Entity> : iRepository<Tbl_Entity> where Tbl_Entity : class
    {
        ...
        public Tbl_Entity GetFirstorDefaultVyParameter(Expression<Func<Tbl_Entity, bool>> wherePredict)
        {
        ...
        public IEnumerable<Tbl_Entity> GetResultBySqlprocedure(string query, params object[] parameter)
        {
            if(parameter != null)
            {
                return _DBEntity.Database.SqlQuery<Tbl_Entity>(query, parameter).ToList();
            }
            else
            {
        ...
        public void RemoveByWhereClause(Expression<Func<Tbl_Entity, bool>> wherePredist)
        {
            Tbl_Entity entity = _dbSet.Where(wherePredist).FirstOrDefault();
            Remove(entity);
        }
